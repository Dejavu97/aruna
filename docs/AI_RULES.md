...
4. **Upload Limits:**
   - Cloudinary uploads use the signed server authorization flow in `api/create-invitation.js`; validate size (max 8MB) and type (gambar png/jpg/gif/webp/svg & audio mp3/wav/ogg) server-side and client-side. API secret remains server-only.
5. **Anti-DoS:**
